-- ==========================================
-- FINANCE APP - ENHANCED SCHEMA v2.0
-- ==========================================
...
BEGIN
  ...
  INSERT INTO public.categories (user_id, name, type, icon, color) VALUES 
    ...
    (new.id, 'Кафе/Рестораны', 'expense', '☕', '#8b5cf6'),
    (new.id, 'Развлечения', 'expense', '🎬', '#ec4899'),
    (new.id, 'Здоровье', 'expense', '💊', '#14b8a6'),
    (new.id, 'Образование', 'expense', '📚', '#6366f1'),
    (new.id, 'Дом', 'expense', '🏠', '#0ea5e9'),
    (new.id, 'Покупки', 'expense', '🛍️', '#a855f7'),
    (new.id, 'Перевод', 'transfer', '🔄', '#64748b');
  
  -- 3. Приветственное уведомление
  INSERT INTO public.notifications (user_id, title, message, type) VALUES 
    (new.id, 'Добро пожаловать! 🎉', 'Начните с создания счета и первой транзакции', 'success');
